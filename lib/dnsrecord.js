...
// DNSRecord.toBuffer = function (record, withLength, outer) {
//   withLength = withLength || false;
//   debug('toBuffer() type: %s, flag:%d class:%s, withLength:%s', record.typeName, record.flag,
//     record.className, withLength);
//   var out = new BufferWriter();
//   //TODO:if outer and any string in data or name
//   //     can be found there. Do a ref instead.
//   out.name(record.name).out.short(record.type).short(record.class);
//   if (record.data) {
//     debug('writing data. ttl:%d, length:%d', record.ttl, record.data.tell());
//     out.long(record.ttl).buffer(record.data, withLength);
//   }

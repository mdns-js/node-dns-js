...
var BufferWriter = module.exports = function (size) {
  this.buf = Buffer.alloc(size || 512);
  this.buf.fill(0);
  this.offset = 0;
};
 ...
 * byte.
 */
BufferWriter.prototype.name = function (v, txt) {
  var self = this;
  debug('#name', v);
  var ref;
  var i;
  var j;
  var part;
  var parts = txt ? [ v ] : v.split('.');
  var parts2 = txt ? [ v ] : v.split('.');
  ...
  if (v.length > 0) {
    for (i = 0; i < parts.length; i++) {
      ...
      part = Buffer.from(parts[i]);
      qap.set(Buffer.concat([ Buffer.from([ part.length ]), part ]));

...
BufferWriter.prototype.buffer = function (v, withLength) {
  ...
  if (v.length > 0) {
    if (withLength) {
      debug('buffer with length', v.length);
      this.short(v.length);
    }
 ...
 * byte.
 */
BufferWriter.prototype.name = function (v, withLength) {
  var self = this;
  debug('#name', v);
  var ref;
  var i;
